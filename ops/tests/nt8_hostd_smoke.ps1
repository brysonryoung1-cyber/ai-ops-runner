# Lightweight integration test for openclaw-nt8-hostd.
# Starts hostd on a random localhost port, calls run/status/collect with fixture topk,
# asserts done.json exists and exit_code eq 3 (Phase-0 stub).
# Requires: .NET 8, pwsh, Python (for tier2 entrypoint). Best run on Windows.
param(
...
# Start hostd in background (dotnet run)
...
try {
    ...
    Write-Host "done.json exists, exit_code=3"
    ...
    Write-Host "nt8_hostd smoke PASS"
}
finally {
