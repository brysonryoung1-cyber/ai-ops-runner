...
# --- Verify mode (--verify: read-only status check, no changes) ---
if [ "${1:-}" = "--verify" ]; then
  _vrc=0
  _vtr="${OPENCLAW_TEST_ROOT:-}"
  if ! command -v tailscale >/dev/null 2>&1; then
    echo "  FAIL: tailscale not found"; exit 1
  fi
  _vip="$(tailscale ip -4 2>/dev/null | head -n1 | tr -d '[:space:]')" || true
  if [ -z "$_vip" ]; then
    echo "  FAIL: Tailscale IPv4 not available"; exit 1
  fi
  echo "  Tailscale IPv4: $_vip"
  _vc="${_vtr}/etc/ssh/sshd_config.d/99-tailscale-only.conf"
  if [ -f "$_vc" ]; then
    if grep -q "ListenAddress $_vip" "$_vc" && grep -q "AddressFamily inet" "$_vc"; then
      echo "  Config: PASS"
    else
      echo "  Config: FAIL (content mismatch)"; _vrc=1
    fi
  else
    echo "  Config: FAIL (not found)"; _vrc=1
  fi
  if command -v ss >/dev/null 2>&1; then
    _vb="$(ss -lntp 2>/dev/null | grep ':22 ' || true)"
    if echo "$_vb" | grep -qE '0\.0\.0\.0:22|\[::\]:22|\*:22'; then
      echo "  Binding: FAIL (public address)"; _vrc=1
    elif echo "$_vb" | grep -q "$_vip:22"; then
      echo "  Binding: PASS (Tailscale-only)"
    else
      echo "  Binding: WARN (not on $_vip:22)"
    fi
  fi
  for _su in ssh.socket sshd.socket; do
    if systemctl is-active --quiet "$_su" 2>/dev/null; then
      echo "  $_su: FAIL (active)"; _vrc=1
    fi
  done
  [ "$_vrc" -eq 0 ] && echo "  RESULT: PASS" || echo "  RESULT: FAIL"
  exit "$_vrc"
fi
...
TS_READY_TIMEOUT="${OPENCLAW_TS_READY_TIMEOUT:-60}"
TS_IP=""
TS_WAITED=0
while [ -z "$TS_IP" ] && [ "$TS_WAITED" -lt "$TS_READY_TIMEOUT" ]; do
  _ts_rc=0
  TS_IP="$(tailscale ip -4 2>/dev/null | head -n1 | tr -d '[:space:]')" || _ts_rc=$?
  if [ -n "$TS_IP" ]; then break; fi
  if [ "$_ts_rc" -eq 0 ]; then break; fi
  if [ "$TS_WAITED" -eq 0 ]; then
    echo "  Tailscale not ready — waiting up to ${TS_READY_TIMEOUT}s..."
  fi
  sleep 2
  TS_WAITED=$((TS_WAITED + 2))
done
if [ -z "$TS_IP" ]; then
  echo "ERROR: Could not determine Tailscale IPv4 address after ${TS_READY_TIMEOUT}s." >&2
  echo "  Is Tailscale up? Run: tailscale status" >&2
  exit 1
fi
if [ "$TS_WAITED" -gt 0 ]; then
  echo "  Tailscale became ready after ${TS_WAITED}s"
fi
...
CONFIG_CHANGED=true
if [ -f "$SSHD_CONF" ]; then
  _existing="$(grep -v '^#' "$SSHD_CONF" | grep -v '^[[:space:]]*$' || true)"
  _desired="$(printf 'AddressFamily inet\nListenAddress %s' "$TS_IP")"
  if [ "$_existing" = "$_desired" ]; then
    CONFIG_CHANGED=false
    echo "  $SSHD_CONF already correct — skipping write"
  fi
fi
...
# --- 6. Restart sshd (skip if config unchanged and already bound correctly) ---
echo ""
echo "--- Step 6: Restart sshd ($SSHD_UNIT) ---"
NEED_RESTART=true
if [ "$CONFIG_CHANGED" = "false" ]; then
  if command -v ss >/dev/null 2>&1; then
    _pre_binds="$(ss -lntp 2>/dev/null | grep ':22 ' || true)"
    if echo "$_pre_binds" | grep -q "$TS_IP:22" && \
       ! echo "$_pre_binds" | grep -qE '0\.0\.0\.0:22|\[::\]:22|\*:22'; then
      NEED_RESTART=false
      echo "  sshd already bound to $TS_IP:22 only — skipping restart (idempotent)"
    fi
  fi
fi
