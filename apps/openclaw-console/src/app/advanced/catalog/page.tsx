...
function CatalogPageContent() {
  ...
  const [confirmTarget, setConfirmTarget] = useState<{ type: "playbook" | "action"; id: string; label: string } | null>(null);
  ...
  const filteredPlaybooks = useMemo(() => {
    if (terms.length === 0) return playbooks;
    return playbooks.filter((playbook) =>
      matchesFilter(
        `${playbook.project_id} ${playbook.title} ${playbook.description} ${playbook.tags.join(" ")}`.toLowerCase(),
        terms
      )
    );
  }, [playbooks, terms]);
  const filteredActions = useMemo(() => {
    if (terms.length === 0) return RAW_ACTIONS;
    return RAW_ACTIONS.filter((action) =>
      matchesFilter(`${action.title} ${action.description} ${action.tags.join(" ")}`.toLowerCase(), terms)
    );
  }, [terms]);
  ...
  const runPlaybook = async (playbookId: string, confirmPhrase?: string) => {
    ...
    if (res.status === 409 && data.error_class === "BREAK_GLASS_REQUIRED") {
      setConfirmTarget({ type: "playbook", id: playbook.id, label: playbook.title });
      setConfirmValue("");
      return;
    }
  ...
  const runAction = async (actionId: string, requiresConfirm: boolean) => {
    if (requiresConfirm) {
      ...
      setConfirmTarget({ type: "action", id: actionId, label: action?.title || actionId });
      setConfirmValue("");
      return;
    }
  ...
  const submitConfirm = async () => {
    if (confirmValue !== "RUN" || !confirmTarget) return;
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="text-2xl font-bold text-white/95 tracking-tight mt-2">Catalog</h2>
          <p className="text-sm text-white/60 mt-1">
            Search playbooks and raw executor actions. Risk badges are policy-first, not cosmetic.
          </p>
        </div>
        <input
          value={query}
          ...
          className="w-full lg:w-80 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-white/35 outline-none focus:border-blue-400/50"
        />
      </div>
            ...
            <div key={playbook.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-white/45 mt-1">{playbook.project_id}</p>
                </div>
              ...
              <p className="text-sm text-white/60 mt-3">{playbook.description}</p>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-4">
          <h3 className="text-lg font-semibold text-white/90">Raw Actions</h3>
          <span className="text-xs text-white/45">{filteredActions.length} matches</span>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filteredActions.map((action) => {
            const requiresConfirm = action.tier === "destructive_ops";
            return (
              <div key={action.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-white/45 mt-1">{action.id}</p>
                  </div>
                  <span className="rounded-full bg-white/5 px-2 py-0.5 text-[10px] text-white/60">
                    {rawTierLabel(action.tier)}
                  </span>
                </div>
                <p className="text-sm text-white/60 mt-3">{action.description}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {action.tags.map((tag) => (
                ...
                <div className="mt-4">
                  <button
                    ...
                    onClick={() => runAction(action.id, requiresConfirm)}
      ...
      {confirmTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#0b1220] p-6 shadow-2xl">
            <p className="text-lg font-semibold text-white/95">Break-glass confirm</p>
            <p className="text-sm text-white/60 mt-2">
              {confirmTarget.label} requires an explicit break-glass confirmation. Type <code>RUN</code> to continue.
            </p>
            <input
              value={confirmValue}
              onChange={(event) => setConfirmValue(event.target.value)}
              className="mt-4 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-white/35 outline-none focus:border-red-400/50"
