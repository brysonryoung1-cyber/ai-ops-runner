 ...
 *
 * Disable autopilot by executing the allowlisted "autopilot_disable" action via hostd.
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await executeAction("autopilot_disable");
    return NextResponse.json(
      { ok: result.ok, stdout: result.stdout, stderr: result.stderr, exitCode: result.exitCode },
