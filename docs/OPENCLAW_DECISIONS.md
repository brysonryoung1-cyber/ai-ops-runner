...
- **Phase model (soma_kajabi)** | Phase 0: read-only only. Phase 1: plan-only (stub). Phase 2+: write actions blocked unless explicitly advanced in project_state. | 2026-02-17 | project-scoped phase gate blocks disallowed actions with error_class PHASE_GATE_BLOCKED.
- **Kill switch (soma_kajabi)** | kill_switch=true blocks all soma_kajabi actions with error_class KILL_SWITCH_ENABLED. Default true for safety. | 2026-02-17 | Set projects.soma_kajabi.kill_switch=false to enable.
- **No-delete policy** | Phase 0 artifacts are never deleted by automation. | 2026-02-17 | Manual cleanup only.
